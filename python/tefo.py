...
pevs, ievs, tevs, trigg_evs = get_pixel_clock_events(dfns, stimtype=stimtype)
...
if off_trigger:
    end_file_idxs.append(len(evs)-1) # add the last trigger event for end-of-file
...
for sidx,eidx in zip(new_file_idxs, end_file_idxs):
    ...
    ard_frame_tval = ard_codes[sidx:eidx]
    ard_frame_ttime = ard_times[sidx:eidx]
...
if no_ard is True:
    ...
    pydict['frame_trigger_times'] = frame_trigger_times
