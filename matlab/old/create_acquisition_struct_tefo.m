...
dsoptions = DSoptions(...
    ...
    'maskpath', '/nas/volume1/2photon/RESDATA/TEFO/20161219_JR030W/em7_centroids/centroids_EM.mat',...
    'maskdims', '3D',...                                        % dimensions of masks
    'maskshape', '3Dcontours',...                               % shape of masks
    'maskfinder', 'centroids',...                                 % method of finding masks, given set of seed coords
    'memmapped', 1,...
    'correctbidi', 0,...
    'slices', [9:30],...                                        % slices from acquis. that actually contain data
    'averaged', false,...                                        % using tiffs that are the averaged tcourses of runs
    'matchedtiffs', {},... %[{[1,1]}, {[2,3]}, {[3,2]}, {[4,4]}],...                                      % matched tiffs, if averaging
    'excludedtiffs', [],...                                     % idxs of tiffs to exclude from analysis
    'metaonly', true,...                                       % only get meta data from tiffs (if files too large)
...
switch dsoptions.roitype
    ...
    case '3Dcnmf'
        ...
        roiparams.tau = [2,2,1] %[2,2,1];                    % std of gaussian kernel (size of neuron) 
        roiparams.p = 2; % 2;                            % order (p = 0 no dynamics, p=1 just decay, p = 2, both rise and decay)
