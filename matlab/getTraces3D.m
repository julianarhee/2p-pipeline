...
data_fns = dir(fullfile(D.mempath, '*.mat')); % Load memmaped movie files
...
data = matfile(fullfile(D.nmfPath, data_fns(fidx).name)); %This is equiv. to 'maskstruct' in 2d
...
centers = com(maskstruct.maskmat,d1,d2,d3);
if size(centers,2) == 2
    centers(:,3) = 1;
end
