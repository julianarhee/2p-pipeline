function [header,Aout,imgInfo] = opentifmeta(varargin)
%% OPENTIF   
...
%
    ...
    %[fileHeader, frameDescs] = scanimage.util.private.getHeaderDataMeta(hTif);
    [fileHeader, frameDescs] = getHeaderDataMeta(hTif);
...
%         header = scanimage.util.private.parseFrameHeaders(fileHeader,frameDescs,verInfo);
        header = scanimage.util.private.parseFrameHeaders(fileHeader,frameDescs(1),verInfo);
...
%         [Aout,imgInfo] = scanimage.util.private.streamOutputQuit(hTif,numImages,si_ver);
        [Aout,imgInfo] = scanimage.util.private.streamOutputQuit(hTif,1,si_ver);
        return;
    end
