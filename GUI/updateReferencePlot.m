...
if newReference==1
    ...
    setappdata(handles.roigui, 'masks', masks);
    
    setRoiMax(handles, masks);                      % Set Max values for ROI entries.

    [RGBimg, masksRGBimg] = createRGBimg(avgimg, masks);         % Create RGB image for average slice and masks.
...
masks = getappdata(handles.roigui,'masks');
D.masksRGBimg(:,:,1) = D.masksRGBimg(:,:,1).*0;
D.RGBimg(:,:,1) = D.RGBimg(:,:,1).*0;
D.RGBimg(:,:,3) = D.RGBimg(:,:,3).*0;
    
if selectedROI>0
    D.masksRGBimg(:,:,1) = D.masksRGBimg(:,:,1)+0.7*masks(:,:,selectedROI);
    D.RGBimg(:,:,1) = D.RGBimg(:,:,1)+0.7*masks(:,:,selectedROI);
    D.RGBimg(:,:,3) = D.RGBimg(:,:,3)+0.7*masks(:,:,selectedROI);
end
