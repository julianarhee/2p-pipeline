...
def do_analysis(options):
	...
	for tiff_count, tiff_fn in enumerate(tiff_files):    	    
	    ...
	    if RETINOID['PARAMS']['roi_type'] == 'pixels':
	        ...
	        plt.savefig(os.path.join(fig_dir,fig_name))
	        plt.close()
	    #delete some file-specific variable values to free some memory   
	    del fourier_data, mag_data, phase_data, freqs, mag_array, phase_array, tmatrix, beta_array, varexp_array,\
	    phimatrix, Xmatrix
