...
def do_regression(t,phi,roi_trace,npixels,tpoints, signal_fit_idx):
...
def analyze_tiff(tiff_path_full,tiff_fn,stack_info, RETINOID,file_dir, fig_dir):
	...
	if os.path.isfile(os.path.join(file_dir,data_fn)): #check if we already analyzed this tiff
		...
		#TODO: implement alternative actions for roi-based analysis
		...
		varexp_array, beta_array, signal_fit = do_regression(t,phi,roi_trace,roi_trace.shape[0],roi_trace.shape[1], max_mod_idx)
...
def do_analysis(options):
	...
	for tiff_count, tiff_fn in enumerate(tiff_files):    	    
		...
		fig_dir = os.path.join(fig_base_dir, stack_info['stimulus'])
		if not os.path.exists(fig_dir):
				os.makedirs(fig_dir)

		analyze_tiff(tiff_path_full,tiff_fn,stack_info, RETINOID,file_dir,fig_dir)
