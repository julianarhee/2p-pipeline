...
def apply_masks_to_movies(TID, RID, output_filedir='/tmp'):
    ...
    try:
        for tfn in tiff_files:
            ...
            if curr_file in roiparams['excluded_tiffs']:
                print "***Skipping %s -- excluded from ROI set %s" % (curr_file, RID['roi_name'])
...
#%%
filetrace_dir = apply_masks_to_movies(TID, output_filedir=filetrace_dir)
