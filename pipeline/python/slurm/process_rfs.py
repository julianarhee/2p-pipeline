...
for mi, mtup in enumerate(meta_list):
    info("... %s" % '|'.join(m))
...
for (animalid, session, fov, experiment, traceid) in meta_list:
    mtag = '-'.join([animalid, session, fov, experiment])
    cmd = "sbatch --job-name=rfs.{PROCID}.{MTAG} \
		-o 'log/rfs.{PROCID}.{MTAG}.out' \
		-e 'log/rfs{PROCID}.{MTAG}.err' \
