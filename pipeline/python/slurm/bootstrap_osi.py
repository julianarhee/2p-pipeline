...
for (visual_area, datakey, animalid, session, fov), g in dsets.groupby(['visual_area', 'datakey', 'animalid', 'session', 'fov']):
    ...
    /n/coxfs01/2p-pipeline/repos/2p-pipeline/pipeline/python/slurm/bootstrap_osi.sbatch \
    {traceid} {rtest} {animalid} {session} {fov}".format(
        procid=piper, mtag=mtag, logdir=logdir,
        exp=experiment, traceid=traceid, rtest=responsive_test, 
        animalid=animalid, session=session, fov=fov) 
    #
