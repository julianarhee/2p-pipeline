...
parser.add_argument('-d', '--response', action='store', dest='response_type', default='dff', help='Response type to use (default: dff)')
...
# Set LOGS
logdir = 'LOG_%s__%s%s_%s-%s_%s_%s_%s' % (analysis_type, match_str, str(visual_area), response_type, trial_epoch, responsive_test, overlap_str, test_str) 
...
def load_metadata(experiment, responsive_test='nstds', responsive_thr=10.,
    ...
    sdata_exp = sdata[(sdata['experiment']==experiment) & (sdata.visual_area.isin(visual_areas))] 
...
def get_cells(experiment='blobs', overlap_thr=None, traceid='traces001', responsive_test='ROC', responsive_thr=0.05, 
            trial_epoch='stimulus', response_type='dff', images_only=True, visual_areas=['V1', 'Lm', 'Li']):
    from pipeline.python.classifications import aggregate_data_stats as aggr
    print("E:", experiment)
    ND, CELLS = aggr.get_final_data(experiment=experiment, overlap_thr=overlap_thr, traceid=traceid, 
                        responsive_test=responsive_test, responsive_thr=responsive_thr, 
                        response_type=response_type, drop_repeats=drop_repeats, trial_epoch=trial_epoch,
                        images_only=images_only)
    return CELLS[CELLS.visual_area.isin(visual_areas)]
...
if analysis_type=='by_ncells':
    ...
    for visual_area in visual_areas:
        for ncells in sample_sizes: #ncells_test:
            ...
            if drop_repeats:
                ...
                -e '{LOGDIR}/{PROCID}.{ANALYSIS}.{MTAG}.{RTEST}.err' \
        /n/coxfs01/2p-pipeline/repos/2p-pipeline/pipeline/python/slurm/decode_by_ncells_drop.sbatch \
        {EXP} {TRACEID} {RTEST} {OVERLAP} {ANALYSIS} {CVAL} {VAREA} {NCELLS} {DKEY} {EPOCH} {TEST} {PASS} {NITER} {DFF}".format(
    ...
    for (visual_area, datakey), g in dsets.groupby(['visual_area', 'datakey']):
        if (analysis_type=='split_pupil') and (datakey.split('_')[1] in old_rats) and (overlap_thr is not None):
            info("--- skipping %s" % datakey)
            continue

        mtag = '%s_%s_%s_%s' % (visual_area, datakey, response_type, C_str) 
    ...
    for (visual_area, datakey), g in dsets.groupby(['visual_area', 'datakey']):
        ...
        {exp} {traceid} {rtest} {overlap} {analysis} {cval} {varea} {ncells} {dkey} {epoch} {test} {niter} {dff}".format(
            procid=piper, mtag=mtag, logdir=logdir,
            exp=experiment, traceid=traceid, analysis=analysis_type,
            rtest=responsive_test, overlap=overlap_thr, 
            cval=c_value, varea=visual_area, ncells=ncells, dkey=datakey, epoch=trial_epoch,
