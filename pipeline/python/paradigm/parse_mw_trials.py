...
paradigm_dir = os.path.join(run_dir, raw_foldername, 'paradigm_files')
raw_files = [f for f in os.listdir(paradigm_dir) if 'mwk' in f or 'serial' in f]

mwfiles = [m for m in raw_files if m.endswith('.mwk')]
...
mw_dfns = [os.path.join(paradigm_dir, f) for f in mwfiles if f.endswith('.mwk')]
...
for didx in range(len(mw_dfns)):
    ...
    if stimtype=='bar':
        ...
        for trialidx,(stim,iti) in enumerate(zip(sorted(stimevents, key=get_timekey), sorted(post_itis, key=get_timekey))):
            ...
            trial[trialnum]['stimuli'] = {'stimulus': stimname, 'position': stimpos, 'scale': stimsize, 'type': stimtype}
