...
xres = list(set(np.diff(gdfs[rf_exp_name].fitinfo['col_vals'])))[0]
yres = list(set(np.diff(gdfs[rf_exp_name].fitinfo['row_vals'])))[0]
#sigma_scale = 2.36
...
fovinfo = coords.get_roi_fov_info(masks, zimg, roi_list=rf_rois)
#fov_pos_x, rf_xpos, xlim, fov_pox_y, rf_ypos, ylim = coords.get_roi_position_um(roi_contours,
#                                                                         rf_exp_name=rf_exp_name,
#                                                                         convert_um=True)

pl.figure()
pl.scatter(gdfs[rf_exp_name].fits['x0'], fovinfo['positions']['ml_pos'])
...
for roi in sorted(gdfs[rf_exp_name].rois):
    _, sx, sy, th, x0, y0 = gdfs[rf_exp_name].fits.loc[roi]
    ...
    rf_avg_size.append(np.mean([abs(sx)*sigma_scale/1., abs(sy)*sigma_scale/1.]))
...
ax2b.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
...
ctx_ap = fovinfo['positions']['ap_pos']
ctx_ml = fovinfo['positions']['ml_pos']
rf_ap = gdfs[rf_exp_name].fits['y0']
rf_ml = gdfs[rf_exp_name].fits['x0']

ax3a = pl.subplot2grid((3, 4), (1, 0), colspan=1, rowspan=1)
ax3a.scatter(ctx_ml, rf_ml, c=colors, alpha=0.3) # FOV y-axis is left-right on brain
...
ax3b.scatter(ctx_ap, rf_ap, c=colors, alpha=0.3) # FOV x-axis is posterior-anterior on brain
