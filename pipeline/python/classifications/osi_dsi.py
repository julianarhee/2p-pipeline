...
def hist_gratings_stats(statsdf, colorvals, metric='meanstim', ax=None,
...
def calculate_gratings_stats(animalid, session, fov, run, traceid, 
                             metric_type='snr', response_thr=1.2,
                             metric_osi='dff',
...
#    print("%i out of %i cells meet min req. of %.2f" % (len(roi_list), avg_resp_by_condition.shape[1], response_thr))
    ...
    roi_list = [k for k, g in gdf if g.groupby(['config']).mean()[metric_type].max() >= response_thr]
    ...
    for roi in roi_list:
        ...
        r_pref = mean_responses.loc[best_cfg]
        r_null = float(mean_responses.loc[null_cfg])
        sigma = np.mean(np.diff(curr_oris)) #/ 2.
        C_offset = np.mean([mean_responses.loc[c] for c in curr_cfgs if c not in [best_cfg, null_cfg]]) #r_pref*-1 #0 #mean_responses.min()
        init_params = [r_pref, r_null, theta_pref, sigma, C_offset]
    
        #fit = spopt.leastsq( double_gaussian_fit, init_params)
...
#            new_length = 20
#            oris_interp = []
#            for orix, ori in enumerate(curr_oris[0:-1]):
#                if ori == curr_oris[-2]:
#                    oris_interp.extend(np.linspace(ori, curr_oris[orix+1], endpoint=True, num=new_length+1))
#                else:
#                    oris_interp.extend(np.linspace(ori, curr_oris[orix+1], endpoint=False, num=new_length))                
#    #        x_fit = np.array(oris_interp).copy()
#            x_fit = np.array(oris_interp).copy()        
#            y_fit = double_gaussian2( x_fit, fit[0] )
#        else:
#            y_fit = double_gaussian( curr_oris, fit[0] )
#            x_fit = curr_oris.copy()
#    
        try:
            
            popt, pcov = spopt.curve_fit(double_gaussian, x, y, p0=init_params, maxfev=1000)
        ...
        if make_plots:
            ...
            if success:
                if plot_interpolate:
                    ...
                    y_plot = double_gaussian( x_plot, *popt)
                else:
                    x_plot = x.copy()
                    y_plot = fitr.copy()
                ax.plot(x_plot, y_plot, c=fit_color)
                ax.text(0, ax.get_ylim()[-1]*0.75, 'r2=%.2f' % r2, fontsize=6)
            else:            
            ...
            if success:
                ...
                radii = double_gaussian( x_plot_polar, *popt)
                thetas = np.array([np.deg2rad(c) for c in x_plot_polar])
                ax.plot(thetas, radii, color=fit_color)
    ...
    print("--- FITS complete ---")
    print("%i out of %i responsive cells (%s, thr: %.2f) appear orientation selective" % (len(fit_rois), len(roi_list), metric_type, response_thr))
    ...
    osi_results_fpath = 'RESULTS_%s.pkl' % osi_dsi_str
    
    with open(os.path.join(output_dir, osi_results_fpath), 'wb') as f:
    ...
    if statsdf.shape[0] > 0:
        hist_gratings_stats(statsdf, colorvals, metric='zscore', ax=ax) 
    ax.set_title('direction selectivity', fontsize=18)
    
    label_figure(fig, data_identifier)
    
    figname = 'hist_dir_selectivity'
    pl.savefig(os.path.join(output_dir, '%s.png' % figname))
    pl.close()
    
    fig, axes = pl.subplots(1, 3) #figure()
    ax = axes[0]
    sns.distplot(list(statsdf['OSI']), ax=ax)
    ax.set_title('OSI')
    
    ax = axes[1]
    sns.distplot(list(statsdf['DSI']), ax=ax)
    ax.set_title('DSI')

    ax = axes[2]
    sns.distplot(list(statsdf['OSI_cv']), ax=ax)
    ax.set_title('circ var')
    
    figname = 'distN_osi_dsi_circvar'
    pl.savefig(os.path.join(output_dir, '%s.png' % figname))
    pl.close()
    ...
    if plot_overlay:
        roi_dir = '/n/coxfs01/2p-data/JC084/20190522/ROIs/rois001_efa39d'
        overlay_osi_rois(statsdf, roi_dir, oris, cmap=cmap, output_dir=output_dir)
