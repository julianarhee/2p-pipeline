...
options = ['-D', '/n/coxfs01/2p-data', '-i', 'JC015', '-S', '20180919', '-A', 'FOV1_zoom2p0x',
           '-R', 'combined_blobs_static', '-t', 'traces003',
           '-r', 'visual', '-d', 'stat', '-s', 'meanstim',
...
def load_classifier_object(animalid, session, acquisition, run, traceid, rootdir='/n/coxfs01/2p-data'):
    found_transform_classifiers = sorted(glob.glob(os.path.join(rootdir, animalid, session, acquisition, run, 'traces', '%s*' % traceid, 'classifiers', 'LinearSVC*', 'TransformClassifier.pkl')), key=natural_keys)
...
def plot_confusion_matrix(cmatrix, classes,
    ...
    pl.colorbar(im, cax=cax, clim=0, cmax=cmax)

    return fig

#%%

C = load_classifier_object(optsE.animalid, optsE.session, optsE.acquisition, optsE.run, optsE.traceid)
...
for clf in C.classifiers:
    ...
    rfe[clf_key] = load_RFE_results(clf)

for clf_key, res in rfe.items():
    ...
    print " --->", kept_rids
    rfe[clf_key]['best'] = {'iter_ix': best_iter,
                            'kept_rids': kept_rids,
...
kept_rids = np.array([i for i in np.arange(0, cX.shape[-1]) if i not in removed_rids])
...
#curr_clf = 'ypos_n13.0'
curr_clf = RFE.keys()[0]
kept_rids = RFE[curr_clf]['best']['kept_rids']
...
clf_output_dir = os.path.join(clf.classifier_dir, 'best_RFE')
if not os.path.exists(os.path.join(clf_output_dir, 'figures')): os.makedirs(os.path.join(clf_output_dir, 'figures'))

#%%

clf_output_dir = os.path.join(clf.classifier_dir, 'fit%i_RFE' % nfeatures_select)
if not os.path.exists(os.path.join(clf_output_dir, 'figures')): os.makedirs(os.path.join(clf_output_dir, 'figures'))
...
#% OK, let's fit this shit:
X_train, X_test, y_train, y_test = train_test_split(cX, cy, test_size=0.50, random_state=0, shuffle=True)
...
pl.figure()
morph_choices = [prob_choose_106[m] for m in morph_levels]
pl.plot(morph_levels, morph_choices)
pl.ylabel('perc. chose 106')
pl.xlabel('morph level')
pl.savefig(os.path.join(clf_output_dir, 'figures', 'perc_choose_106.png'))
